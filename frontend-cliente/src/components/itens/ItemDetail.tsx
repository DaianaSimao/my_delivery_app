...
import { ArrowLeft, Share2, ChevronDown, ChevronUp, Plus, Minus } from 'lucide-react';
import type { MenuItem, Option, OptionGroup } from '../../types';

interface ItemDetailsProps {
  item: MenuItem;
  onBack: () => void;
  onAddToCart: (item: MenuItem, selectedOptions: Record<number, number>, observation: string) => void;
}

const ItemDetails: React.FC<ItemDetailsProps> = ({ item, onBack, onAddToCart }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [observation, setObservation] = useState('');
  const [expandedGroups, setExpandedGroups] = useState<number[]>([]);
  const [selectedOptions, setSelectedOptions] = useState<Record<number, number>>({});

  // Dados fictícios de grupos de opções (substitua por dados da API)
  ...
  }, item.price); // Preço base do item

  const handleAddToCart = () => {
    onAddToCart(item, selectedOptions, observation);
  };
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-900">
      ...
      <div className="px-4 py-6 -mt-6 rounded-t-3xl bg-white dark:bg-gray-900 relative">
        ...
        <p className="text-lg font-semibold text-red-600 dark:text-red-400 mt-2">R$ {item.price.toFixed(2)}</p>
