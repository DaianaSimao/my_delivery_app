...
const Restaurantes: React.FC = () => {
  ...
  const handleRestaurantesClick = () => {
    navigate("/restaurantes/new");
  };

  const handleEditarClick = (restauranteId: number) => {
    navigate(`/restaurantes/${restauranteId}/editar`);
  };
  ...
  useEffect(() => {
    const fetchRestaurantes = async () => {
      try {
        ...
        setTotalRestaurantes(response.data.meta.total_count);
        setLoading(false);
      } catch (err) {
        setError("Erro ao carregar restaurantes. Verifique sua autenticação.");
        setLoading(false);
        console.error(err);
      }
    };
        ...
        <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
            <div className="flex-1 flex items-center space-x-2">
              <h5>
                <span className="text-gray-500">Todos os restaurantes: </span>
                <span className="dark:text-white"> {totalRestaurantes} </span>
              </h5>
              ...
              >
                Mostrando 1-{restaurantes.length} de {restaurantes.length} resultados
                ...
                {restaurantes.map((restaurante) => (
                  ...
                  >
                    <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white flex items-center">
                      <img
                        src={restaurante.imagem_url || restauranteIcon}
                    ...
                    <td className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                      <div className="flex items-center space-x-4">
                        <button
                          type="button"
                          onClick={() => restaurante.id !== undefined && handleEditarClick(restaurante.id)}
          ...
          <nav className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4 overflow-x-auto" aria-label="Table navigation">
            <span className="text-sm font-normal text-gray-500 dark:text-gray-400">
              Mostrando
              <span className="font-semibold text-gray-900 dark:text-white"> {((currentPage - 1) * perPage) + 1} - {Math.min(currentPage * perPage, restaurantes.length)} </span>
              de
              <span className="font-semibold text-gray-900 dark:text-white"> {restaurantes.length} </span>
            </span>
